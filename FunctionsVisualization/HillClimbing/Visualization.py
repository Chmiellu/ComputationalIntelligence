...
def schwefel(*args):
    return 418.9829 * len(args) - np.sum(arg * np.sin(np.sqrt(np.abs(arg))) for arg in args)
...
for idx, (point, cost_value) in enumerate(infos):
    if points == 'every10':
        ...
        if idx == 0:
            ax.scatter(point[0], point[1], cost_value, color='blue', label='Start Point', s=50)
        elif idx == len(infos) - 1:
            ax.scatter(point[0], point[1], cost_value, color='red', label='End Point', s=50)
            ax.legend()
